...
void chat_with_server()
{
    while (1)
    {
        ...
        string decode_data = aes_decode(s);
        ...
        string encode_reply = aes_encode(reply);
        //发送
...
void online()
{
    ...
    // 第二步，将使用 RSA 加密后的AES密钥发送给服务端

    string str_key = "12345678";
    key = expandKey(str_key);

    unsigned int* encrpt_des_key = new unsigned int[str_key.length()];
