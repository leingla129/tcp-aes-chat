...
void chat_with_client(int fd)
{
    while (1)
    {
        ...
        string encode_reply = aes_encode(reply);//aes_encode
        ...
        string s = buffer;
        string decode_data = aes_decode(s);//aes_decode
        ...
        if (decode_data[0] == '-' && decode_data[1] == 'e')    //客户端发起exit命令,结束本次通信
            ...
            std::cout << "client：" << decode_data << endl;
    }
}
void do_service(int fd)
{
    ...
    {
        std::stringstream ss;
        ss << Rsa::Decry(encrpt_des_key[i], s_key);
        key_0 += ss.str();
    }
    cout <<"接收AES密钥："<< key_0 << endl;
    //aes_setKey
    key = expandKey(key_0);
